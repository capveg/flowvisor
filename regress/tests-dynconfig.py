...
h = FvRegress.parseConfig(configDir='flowvisor-conf.d-dynconfig', valgrind=valgrindArgs)
if wantPause:
	doPause("before adding switch2")
...
'''
	...
	packet_out_p0_aftr = FvRegress.OFVERSION + '''0d 00 58 05 01 00 00 ff ff ff ff ff ff 00 08
                00 00 00 08 00 00 00 80 00 00 00 00 00 01 00 00
	...
	h.runTest(name="(2nd) packet_out; valid", timeout=timeout, events= [
	...
	stats_reply_desc_after = FvRegress.OFVERSION + ''' 11 03 2c 00 00 00 00 00 00 00 00 4e 45 43 20
	...
	stats_reply_desc_after2= FvRegress.OFVERSION + '''11 03 2c 00 00 00 00 00 00 00 00 4e 45 43 20
	...
	packet_out_p0 =  FvRegress.OFVERSION + '''0d 0058 0000 abcd ffff ffff
			...
			# note, the xid here is a function of the order of the tests;
			#	DO NOT CHANGE test order
	packet_out_p0_aftr_port0 = FvRegress.OFVERSION + '''0d 00 68 0a 01 00 00 ff ff ff ff ff ff 00 18
	...
	packet_out_p1 =  FvRegress.OFVERSION + '''0d 0058 0000 abce ffff ffff
				...
				0000 001e d7c3 cdc0 251b e6dc ea0c 726d
                973f 2b71 c2e4 1b6f bc11 8250'''
	packet_out_p0_aftr_port_all = FvRegress.OFVERSION + '''0d 00 70 0c 01 00 00 ff ff ff ff ff ff 00 20
			...
			'''
	packet_out_p1_aftr = FvRegress.OFVERSION + '''01 000c 0000 abce 0004 0000'''	# NOT correct output -- yet; need to append bad message
	h.runTest(name=" (3rd) packet_out; valid", timeout=timeout, events= [
			TestEvent( "send","guest","bob", packet=packet_out_p0),
			TestEvent( "recv","switch","switch1", packet=packet_out_p0_aftr_port0),
			TestEvent( "send","guest","bob", packet=packet_out_p1),
			TestEvent( "recv","guest","bob", packet=packet_out_p1_aftr),
			TestEvent( "send","guest","bob", actorID2='switch2', packet=packet_out_p0),
			TestEvent( "recv","switch","switch2", packet=packet_out_p0_aftr_port_all),
			TestEvent( "send","guest","bob", actorID2='switch2', packet=packet_out_p1),
			TestEvent( "recv","guest","bob", actorID2='switch2', packet=packet_out_p1_aftr),
			])
	#############################################################
	#  Change the flowvisor's config now!
	print '''
