...
public class FlowEntryTest extends TestCase {
	...
	public void testFlowMatch () {
		...
		TestCase.assertEquals(MatchType.EQUAL, flowEntryAll.matches(1, allmatch).getMatchType());
		TestCase.assertEquals(MatchType.SUBSET, flowEntryAll.matches(1, goodmatch).getMatchType());
		TestCase.assertEquals(MatchType.SUBSET, flowEntryAll.matches(1, badmatch).getMatchType());
		
		FlowEntry flowEntrySpecific = new FlowEntry(1, goodmatch, (OFAction)null);
		TestCase.assertEquals(MatchType.SUPERSET, flowEntrySpecific.matches(1, allmatch).getMatchType());
		TestCase.assertEquals(MatchType.EQUAL, flowEntrySpecific.matches(1, goodmatch).getMatchType());
		TestCase.assertEquals(MatchType.NONE, flowEntrySpecific.matches(1, badmatch).getMatchType());
	...
	public void testCIDRFlowMatch () {
		...
		TestCase.assertEquals(MatchType.SUBSET, flowEntry24.matches(1, l32match).getMatchType());		
		TestCase.assertEquals(MatchType.EQUAL, flowEntry24.matches(1, l24match).getMatchType());
		TestCase.assertEquals(MatchType.SUPERSET, flowEntry24.matches(1, allmatch).getMatchType());
