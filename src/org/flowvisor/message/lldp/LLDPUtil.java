...
public class LLDPUtil {
	 ...
	 */
	static public boolean handleLLDPFromController(FVPacketOut po, FVClassifier fvClassifier, FVSlicer fvSlicer) {
	...
	/**
	 * If this msg is lldp, then
	 * 		1) remove the slice identifying trailer
	 * 		2) send to controller  -- all slices can send lldp, no matter flowspace
	 *      3) return true, we've handled this packet
	 * @param po
	 * @param fvClassifier
	 * @return did we handle this message?
	 */
	static public boolean handleLLDPFromSwitch(FVPacketIn pi, FVClassifier fvClassifier) {
